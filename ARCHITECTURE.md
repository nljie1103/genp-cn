...
> **本文档是项目的核心技术参考**，涵盖：架构总览、AutoIt 语法、模块详解、构建流程、安全分析。  
> 配合源码中的中文注释一起阅读效果最佳。
...
| 风险等级 | 操作 | 说明 |
|---------|------|------|
| 🔴 严重 | 修改系统 DLL | `patch_wintrust.ps1` 修改 Windows 签名验证核心组件 `wintrust.dll`，偏移 `0x1C86-0x1C87` → `0x33 0xC0`（`xor eax, eax`），使签名验证始终返回成功 |
| 🔴 严重 | 管理员权限 | `#RequireAdmin` 要求完整管理员权限：修改 hosts、防火墙、Program Files 下的 Adobe 文件、注册表 |
| 🟡 中等 | 二进制补丁 | `config.ini` → `[Patches]` 直接修改 Adobe 程序机器码（绕过许可验证、过期检查、签名完整性检查等） |
| 🟡 中等 | 网络请求 | 从外部 URL 下载域名屏蔽列表写入系统 hosts 文件 |
| 🟡 中等 | 修改 hosts | 将 Adobe 域名重定向到 `127.0.0.1`，可能影响系统网络解析 |
| 🟡 中等 | 防火墙规则 | 通过 `netsh advfirewall` 创建入站/出站阻止规则 |
| 🟡 中等 | 注册表修改 | DevOverride 写入 `HKLM\SOFTWARE\Adobe\` 键值 |
| ⚪ 低 | UPX 加壳 | 最终产物使用 UPX 压缩，可能导致杀毒误报 |
...
| 后门 / 远程控制 | ❌ **未发现** |
| 数据窃取 / 键盘记录 | ❌ **未发现** |
| 隐藏功能 | ❌ **未发现**（所有功能与界面声明一致） |
| 构建依赖来源 | ✅ 官方网站 `autoitscript.com` |
| 代码逻辑一致性 | ✅ 与声明功能完全吻合 |
...
| 维度 | 结论 |
|------|------|
| 语言 | AutoIt v3 脚本 + PowerShell 构建辅助 |
| 编译 | AutoIt3Wrapper → `.exe` + UPX 压缩 |
| 恶意后门 | 源码中 **未发现** 后门或木马行为 |
| 是否安全 | **不安全** — 修改系统核心 DLL、篡改二进制文件、修改防火墙和 hosts |
| 合法性 | 破解工具，使用构成对 Adobe 许可协议的违反 |

> ⚠️ 源码本身没有恶意后门，但工具的功能本身就是不安全的。
